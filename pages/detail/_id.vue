                ...
                <div class="tags">
                  <div>
                    <span class="layui-badge layui-bg-green">{{
                      catalogObj[postInfo.catalog]
                    }}</span>
                    <span class="layui-badge layui-bg-gray">{{
                      postInfo.isEnd === '0' ? '未结' : '已结'
                    }}</span>
                    <span
                      class="layui-badge layui-bg-blue"
                      v-if="postInfo.isTop === '1'"
                      >置顶</span
                    >
                    <span
                      class="layui-badge"
                      v-for="(tag, index) in postInfo.tags"
                      :key="'tag' + index"
                      :class="tag.class"
                      >{{ tag.name }}</span
                    >
            ...
            <div class="post-user">
              <div class="info">
                ...
                <div class="basic">
                  <p class="user">
                    <span class="name">{{
                      postInfo.user ? postInfo.user.name : ''
                    }}</span>
                    <span
                      class="vip"
                      v-if="postInfo.user && postInfo.user.isVip === '1'"
                      ><svg
                        t="1601396379570"
                        class="icon"
                    ...
                    ></span>
                    <span
                      class="layui-badge layui-bg-blue fly-margin-right"
                      v-if="postInfo.user && postInfo.user.isVip === '1'"
                      >VIP{{ postInfo.user ? postInfo.user.isVip : '' }}</span
                    >
                    <span class="date">{{
                      postInfo.created | formatTime
                    }}</span>
                  </p>
              ...
              <div class="operation">
                <router-link
                  ...
                  v-if="
                    postInfo.user &&
                    userInfo &&
                    postInfo.user._id === userInfo._id &&
                    postInfo.isEnd === '0'
                  "
                  :to="{
                    name: 'edit',
                    params: { tid: postInfo._id, postInfo },
                  }"
                  >编辑</router-link
                >
                <button
              ...
              >
                  ...
                  <div class="info">
                    ...
                    <div class="basic">
                      <p class="user">
                        ...
                        <span
                          class="vip"
                          v-if="item.user && item.user.isVip === '1'"
                          ><svg
                            t="1601396379570"
                            class="icon"
                            viewBox="0 0 1024 1024"
                        ...
                        <span
                          class="layui-badge layui-bg-blue"
                          v-if="item.user && item.user.isVip === '1'"
                          >VIP{{ item.user ? item.user.isVip : '' }}</span
                        >
                      </p>
                ...
                <div class="operation">
                  <div>
                    <span
                      ><i
                        ...
                        :style="{
                          color: item.isHands === '1' ? '#009688' : '',
                        }"
                        @click="handsComment(item)"
                      ></i
                      >{{ item.hands }}</span
                    >
                    <span
                  ...
                  <div v-if="userInfo && postInfo.isEnd === '0'">
                    <a
                      v-show="item.user._id === userInfo._id"
                      @click.prevent="editComment(item._id, item.content)"
                      >编辑</a
                    >
                    <a
                      v-show="item.user._id === userInfo._id"
                    ...
                    <a
                      v-show="
                        postInfo.user._id === userInfo._id &&
                        item.user._id !== postInfo.user._id
                      "
                      @click.prevent="bestComment(item)"
                      >采纳</a
                    >
                  </div>
                </div>
            ...
            <form class="layui-form layui-form-pane">
              ...
              <div class="submit">
                <button
                  ...
                  :title="
                    userInfo && userInfo.status !== '0'
                      ? '该用户已被禁言，请联系管理员'
                      : ''
                  "
                  :class="{
                    'layui-btn-disabled': userInfo && userInfo.status !== '0',
                  }"
                >
                  {{ updateCid ? '更新' : '发表' }}评论
                </button>
...
export default {
  ...
  async asyncData({ $axios, params }) {
    const data = await $axios.$get('/public/post/detail?tid=' + params.id)
    return {
      postInfo: data.data,
      tid: params.id,
    }
  ...
  mounted() {
    ...
    this.$axios
      .$get('/public/comments?' + qs.stringify(options))
      .then((res) => {
        if (res.code === 10000) {
          this.comments = res.data.list
          this.total = res.data.total
